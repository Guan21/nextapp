...
contract ETHYieldManager is IYieldManager {
    ...
    function initializeYieldProvider() external {
    ...
    function unstake(
        uint256 amount
    ) external returns (uint256 pending, uint256 claimed) {
        (bool success, bytes memory result) = yieldProviderImplementation
            .delegatecall(abi.encodeWithSignature("unstake(uint256)", amount));
    ...
    function stakedBalance() external view returns (uint256) {
        (bool success, bytes memory result) = yieldProviderImplementation
            .staticcall(abi.encodeWithSignature("stakedBalance()"));
    ...
    function yield() external view returns (int256) {
        (bool success, bytes memory result) = yieldProviderImplementation
            .staticcall(abi.encodeWithSignature("yield()"));
    ...
    function insuranceBalance() external view returns (uint256) {
        (bool success, bytes memory result) = yieldProviderImplementation
            .staticcall(abi.encodeWithSignature("insuranceBalance()"));
