...
abstract contract YieldProvider {
    ...
    /// @notice initialize
    function initialize() external virtual onlyDelegateCall {}
    ...
    function unstake(
        uint256
    ) external virtual returns (uint256 pending, uint256 claimed);
    ...
    function recordStakedDeposit(
        uint256 amount
    ) external virtual onlyYieldManager {
    ...
    function recordUnstaked(
        ...
        if (pending > 0) {
            require(
                claimed == 0 && pending == expected,
                "invalid yield provider implementation"
            );
    ...
    function preCommitYieldReportDelegateCallHook()
        external
        virtual
        onlyDelegateCall
    {}
